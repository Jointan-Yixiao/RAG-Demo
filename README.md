...
这是一个在固定资料集上完成安装、重建索引和真实提问验证的演示项目，不是通用问答产品：它只回答这 10 份资料里的内容，没有网页界面，入口是命令行，答案以 Markdown 文件输出。

## 它能做什么

- **中文口语提问**：可以指定资料（“Lewis 那篇论文里……”），也可以不指定。指定了库里没有的资料时，会给出说明并停下，不会擅自改查其他资料。
- **图文证据**：命中图表时，把原图发给生成模型，并在答案里嵌入原图。
...
| API 密钥 | DeepSeek API key，只在加 `--execute` 时读取 |
...
**测试**：本仓库只携带 19 项运行链路测试，不需要 GPU 或 API：

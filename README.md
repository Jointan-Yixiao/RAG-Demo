...
这是一个在固定资料集上完成安装、重建索引和真实提问验证的演示项目，不是通用问答产品：它只回答这 10 份资料里的内容。现在提供 Windows 图形启动器和本机网页工作台，也保留命令行入口和 Markdown 答案导出。

## 它能做什么

- **桌面启动器**：首次打开自动创建桌面快捷图标，检查环境、一键准备依赖和索引、启动或停止服务。
...
1. 从 GitHub 下载并**解压完整项目**，或用 Git 克隆到准备长期保存的目录。
2. 先满足上面的环境要求：安装 **Python 3.12（含 py 启动器）和 Node.js 24.x**，准备可用的 NVIDIA 驱动。
3. 双击项目里的 **`RAG-Workbench.exe`**，或 **`启动工作台.cmd`**。启动器首次运行会自动在当前用户桌面创建 **“RAG 工作台”** 快捷方式。
4. 点击 **“初始化环境”**，自动创建项目 Python 环境、安装固定依赖、下载 GME / BGE 并重建索引。已有完整环境会直接复用；此步骤需要下载较大文件，**不调用付费模型 API**。
5. 显示就绪后点击 **“打开工作台”**，在网页“设置”中填写自己的 API 密钥，然后提问。

日常直接双击桌面的 **“RAG 工作台”**。关闭网页或启动器后，后台服务仍会保留；用启动器里的 **“停止服务”** 结束它。正在生成回答时会拒绝停止。图标被误删时，可以点击 **“修复桌面图标”**。

**下载 ZIP 或 `git clone` 本身不会执行程序。** 自动创建桌面图标发生在首次运行启动器时，无需管理员权限。快捷方式指向项目目录，请保留整个文件夹；移动项目后，从新位置运行启动器并点击“修复桌面图标”。

<img src="assets/app.png" alt="资料与检索启动器图标" width="96" height="96">

程序和图标源码分别在 [`launcher/WorkbenchLauncher.cs`](launcher/WorkbenchLauncher.cs) 与 [`assets/app.svg`](assets/app.svg)。不需要 Node 前端构建或额外安装程序。首次初始化中断会保留已下载内容；错误信息保存在 `data/ui/setup.log`。完整说明见 [启动器说明](doc/启动器实现说明.md)。
...
完成前面环境、模型和索引准备后，双击 **RAG-Workbench.exe** 或 **启动工作台.cmd**，在启动器中点击 **打开工作台**。浏览器会打开 `http://127.0.0.1:8765`。在设置页配置 API 密钥，或沿用项目 `.env` 中的 `DEEPSEEK_API_KEY`。
...
- **桌面启动器验收**：源码编译成功，21 组离线 C# 测试通过；实际运行检查了窗口排版、状态识别与首次自动创建桌面图标。现有完整环境的初始化复用、启动停止和历史证据接口已验证；新电脑空环境下一键下载安装仍待验证。
...
**测试**：本仓库携带 59 项运行链路、工作台与初始化测试，不需要 GPU 或付费 API：
...
- [启动器说明](doc/启动器实现说明.md)：桌面入口、自动初始化、快捷方式与构建方法
